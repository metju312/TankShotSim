...
This file contains information on the IEEE-1516e Portico Java zzzexample federate and how
...
-------------------------------------------------------------------------------
 1. Getting Started
-------------------------------------------------------------------------------
 This zzzexample application provides the basic template code for a very simple
 ...
 The zzzexample federate consists of three main files:
   * src/zzzexample/ExampleFederate.java           // the bulk of the federate code
   * src/zzzexample/ExampleFederateAmbassador.java // zzzexample Federate Ambassador (callbacks)
 ...
 data. When it is run in the same federation as another zzzexample federate, any
...
-------------------------------------------------------------------------------
 ...
 The Portico Java zzzexample federate is officially supported on:
 ...
 the zzzexample federates. Portico ships with a JRE, but this only supports
...
-------------------------------------------------------------------------------
 ...
 Each of the scripts takes the same basic commands:
   ...
   * compile           // Compile the zzzexample federate
   * execute           // Run the zzzexample federate
 ...
 open a command prompt, change into the zzzexample directory and run the batch
 file from there:
 
   * [Open Command Prompt]
   * cd %RTI_HOME%\examples\java\zzzexample
   * windows compile
   * windows execute
 ...
 the "execute" for each of the helper scripts. This will be the federate name:
   ...
   * Start the first zzzexample federate (don't hit enter when it says so)
   * Start a second zzzexample federate
 ...
 Each zzzexample federate will wait until you hit enter to tell the RTI that the
